...
def get_filme_by_id(filme_id):
    query = """
        SELECT 
            f.id_filme, f.titulo, f.ano, f.poster, f.rating, f.synopsis, 
            f.tempo_duracao, f.status, f.id_filme_original,
            l.linguagem,
...
def create_filme(data):
    ...
    tempo_duracao = data.get("tempo_duracao") or None
    ...
    try:
        existing_film = fetch_one(
            "SELECT id_filme FROM Filme WHERE titulo = %s AND ano = %s",
            (titulo, ano),
        )
        if existing_film:
            return {
                "error": f"O filme '{titulo}' ({ano}) já está cadastrado."
            }, 409

        query_filme = """
            INSERT INTO Filme 
            (titulo, ano, poster, rating, synopsis, id_linguagem, tempo_duracao, status)
            VALUES (%s, %s, %s, %s, %s, %s, %s, 'PENDENTE_CADASTRO')
        """
        id_filme = execute_query(
            query_filme,
            (titulo, ano, poster, rating, synopsis, id_linguagem, tempo_duracao),
        )

        for genero_nome in generos_nomes:
            genero_obj = fetch_one(
                "SELECT id_genero FROM Genero WHERE genero = %s", (genero_nome,)
            )
            id_genero = (
                genero_obj["id_genero"]
                if genero_obj
                else execute_query(
                    "INSERT INTO Genero (genero) VALUES (%s)", (genero_nome,)
                )
            )
            execute_query(
        ...
        if diretor_nome:
            diretor_obj = fetch_one(
                "SELECT id_diretor FROM Diretor WHERE nome = %s", (diretor_nome,)
            )
            id_diretor = (
                diretor_obj["id_diretor"]
                if diretor_obj
                else execute_query(
        ...
        for ator_nome in atores:
            if ator_nome:
                ator_obj = fetch_one(
                    "SELECT id_ator FROM Ator WHERE nome = %s", (ator_nome,)
                )
                id_ator = (
                    ator_obj["id_ator"]
                    if ator_obj
                    else execute_query(
                        "INSERT INTO Ator (nome, sobrenome) VALUES (%s, %s)",
                        (ator_nome, ""),
                    )
                )
                execute_query(
...
def create_edit_submission(id_filme_original, data):
    ...
    try:
        query_filme = """
            INSERT INTO Filme
            (titulo, ano, poster, rating, synopsis, id_linguagem, tempo_duracao, status, id_filme_original)
            VALUES (%s, %s, %s, %s, %s, %s, %s, 'PENDENTE_EDICAO', %s)
        """
        id_filme_pendente = execute_query(
            query_filme,
            (
                titulo, ano, poster, rating, synopsis,
                id_linguagem, tempo_duracao, id_filme_original
            ),
        )
...
def get_solicitacoes_cadastro():
    return fetch_all(
        "SELECT * FROM Filme WHERE status = 'PENDENTE_CADASTRO' ORDER BY id_filme ASC"
    )

def get_solicitacoes_edicao():
    return fetch_all(
        "SELECT * FROM Filme WHERE status = 'PENDENTE_EDICAO' ORDER BY id_filme ASC"
    )
    ...
    try:
        ...
        # Aprovação de edição
            ...
            execute_query(
                ...
                """,
                (
                    filme_pendente["titulo"], filme_pendente["ano"],
                    filme_pendente["poster"], filme_pendente["rating"],
                    filme_pendente["synopsis"], filme_pendente["id_linguagem"],
