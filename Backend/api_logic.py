...
def get_filmes(params):
    ...
    # MUDANÇA: Apenas filmes APROVADOS são mostrados na lista principal
...
def get_filme_by_id(filme_id):
    # (Esta função permanece a mesma, busca todos os detalhes)
    query = """
        SELECT 
...
# LÓGICA DE NEGÓCIO PARA FILME (POST - Solicitar Cadastro)
def create_filme(data):
    ...
    try:
        ...
        # MUDANÇA: Salva com status 'PENDENTE_CADASTRO'
...
# LÓGICA DE NEGÓCIO PARA FILME (POST - Solicitar Edição)
def create_edit_submission(id_filme_original, data):
    ...
    try:
        ...
        # (Lógica de Gêneros, Diretor e Atores permanece a mesma para este *novo* filme pendente)
        if generos_nomes:
            for genero_nome in generos_nomes:
                genero_obj = fetch_one("SELECT id_genero FROM Genero WHERE genero = %s", (genero_nome,)); id_genero = genero_obj["id_genero"] if genero_obj else execute_query("INSERT INTO Genero (genero) VALUES (%s)", (genero_nome,))
...
# LÓGICA DE NEGÓCIO PARA FILME (DELETE - Recusar ou Excluir)
def delete_filme(filme_id):
    # Esta função agora é usada para "Recusar" (apagar PENDENTE)
